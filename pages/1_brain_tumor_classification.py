...
CLASS_INFO = {
    "glioma":     "A tumor arising from glial cells in the brain or spine.",
    "meningioma": "A tumor forming on the membranes surrounding the brain and spinal cord.",
...
def load_model():
    try:
        ...
        model = tf.keras.models.load_model("best_brain_model.keras")
        return model, None
...
st.markdown('<div class="subtitle">Upload a brain MRI — the model will predict tumor type using a custom VRF-Net architecture.</div>', unsafe_allow_html=True)
...
if load_error:
    ...
    st.info("Make sure `best_brain_model.keras` is in the same directory as this app.")
    st.stop()
