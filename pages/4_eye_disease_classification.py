...
# -------------------------
CLASS_NAMES = ['Glaucoma', 'Healthy', 'Diabetic Retinopathy', 'Macular Scar', 'Myopia']

CLASS_INFO = {
    "Glaucoma": "Signs of glaucoma, a condition that can damage the optic nerve and affect vision.",
    "Healthy": "No significant eye abnormalities detected; the eye appears healthy.",
    "Diabetic Retinopathy": "Signs of diabetic retinopathy, a diabetes-related retinal disease.",
    "Macular Scar": "Signs of macular scarring that may affect central vision.",
    "Myopia": "Signs associated with myopia (nearsightedness)."
}
...
def load_model():
    try:
        ...
        model = tf.keras.models.load_model("best_eye_model.keras")
        return model, None
...
<div class="stats-bar">
    <div class="stat-item">
        <div class="stat-num">78.88%</div>
        <div class="stat-label">Model Accuracy</div>
    </div>
""", unsafe_allow_html=True)


st.markdown('<div class="main-title">👁️ Eye Disease Classifier</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Upload a eye image — the model will predict eye condition using a custom VRF-Net architecture.</div>', unsafe_allow_html=True)
...
if load_error:
    ...
    st.info("Make sure `best_eye_model.keras` is in the same directory as this app.")
    st.stop()
