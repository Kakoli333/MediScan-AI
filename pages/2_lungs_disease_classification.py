...
# -------------------------
CLASS_NAMES = ['Bacterial Pneumonia', 'Corona Virus Disease', 'Normal', 'Tuberculosis']

CLASS_INFO = {
    "Bacterial Pneumonia":  "A lung infection caused by bacteria leading to inflammation and fluid buildup in the lungs.",
    "Corona Virus Disease": "A viral respiratory infection caused by SARS-CoV-2 affecting the lungs and breathing.",
    "Normal":               "No signs of lung disease detected; lungs appear healthy.",
    "Tuberculosis":         "A contagious bacterial infection that primarily affects the lungs and can cause long-term damage."
}
...
def load_model():
    try:
        ...
        model = tf.keras.models.load_model("best_lungs_model.keras")
        return model, None
...
<div class="stats-bar">
    <div class="stat-item">
        <div class="stat-num">96.86%</div>
        <div class="stat-label">Model Accuracy</div>
    </div>
""", unsafe_allow_html=True)


st.markdown('<div class="main-title">🫁 Lungs Disease Classifier</div>', unsafe_allow_html=True)
st.markdown('<div class="subtitle">Upload a chest x-ray — the model will disease type using a custom VRF-Net architecture.</div>', unsafe_allow_html=True)
...
if load_error:
    ...
    st.info("Make sure `best_lungs_model.keras` is in the same directory as this app.")
    st.stop()

# Upload
uploaded_file = st.file_uploader(
    "Choose a x-ray image",
    type=["jpg", "jpeg", "png"],
